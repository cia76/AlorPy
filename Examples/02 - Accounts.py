...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    for exchange in ap_provider.exchanges:  # Пробегаемся по всем биржам
        securities = ap_provider.GetSecuritiesExchange(exchange)  # Получаем все тикеры на бирже
    ...
    for user_name, refresh_token in ap_providers:  # Пробегаемся по всем счетам
        ...
        portfolios = ap_provider.GetPortfolios()  # Портфели: Фондовый рынок / Фьючерсы и опционы / Валютный рынок
        for p in portfolios:  # Пробегаемся по всем портфелям
            ...
            for exchange in ap_provider.exchanges:  # Пробегаемся по всем биржам
                ...
                positions = ap_provider.GetPositions(portfolio_name, exchange, True)  # Позиции без денежной позиции
                for position in positions:  # Пробегаемся по всем позициям
                    symbol = position['symbol']  # Тикер
                    symbol_info = ap_provider.GetSymbol(exchange, symbol)  # Информация о тикере
                ...
                money = ap_provider.GetMoney(portfolio_name, exchange)  # Денежная позиция
                print(f'  - Позиции {round(money["portfolio"] - money["cash"], 2)} + Свободные средства {money["cash"]} = {round(money["portfolio"], 2)}')
                orders = ap_provider.GetOrders(portfolio_name, exchange)  # Получаем список активных заявок
                ...
                stop_orders = ap_provider.GetStopOrders(portfolio_name, exchange)  # Получаем список активных стоп заявок
