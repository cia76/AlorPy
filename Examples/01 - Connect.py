...
def PrintNewBar(response):
    ...
    dtMsk = datetime.utcfromtimestamp(seconds) if type(tf) is str else apProvider.UTCTimeStampToMskDatetime(response['data']['time'])  # Дневные бары и выше ставим на начало дня по UTC. Остальные - по МСК
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    apProvider2 = AlorPy()  # AlorPy - это Singleton класс. При попытке создания нового экземпляра получим ссылку на уже имеющийся экземпляр
    ...
    tf = 'D'  # 60 = 1 минута, 300 = 5 минут, 3600 = 1 час, D = день, W = неделя, M = месяц, Y = год
    secondsFrom = apProvider.MskDatetimeToUTCTimestamp(datetime(2022, 4, 1))  # С заданной даты/времени МСК, переведенной секунды, прошедших с 01.01.1970 00:00 UTC
