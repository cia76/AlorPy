...
def load_candles_from_file(board='TQBR', symbol='SBER', tf='D1', datapath=os.path.join('..', '..', 'Data', 'Alor', ''),
                           delimiter='\t', dt_format='%d.%m.%Y %H:%M') -> pd.DataFrame:
    ...
    """
    filename = f'{datapath}{board}.{symbol}_{tf}.txt'
...
# noinspection PyShadowingNames
def get_candles_from_provider(ap_provider=AlorPy(), board='TQBR', symbol='SBER', tf='D1', seconds_from=0) -> pd.DataFrame:
...
# noinspection PyShadowingNames
def save_candles_to_file(ap_provider=AlorPy(), board='TQBR', symbols=('SBER',), tf='D1',
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    # board = 'RFUD'  # Фьючерсы
    # symbols = ('SiM4', 'RIM4')  # Формат фьючерса: <Тикер><Месяц экспирации><Последняя цифра года> Месяц экспирации: 3-H, 6-M, 9-U, 12-Z
    # symbols = ('USDRUBF', 'EURRUBF', 'CNYRUBF', 'GLDRUBF', 'IMOEXF')  # Вечные фьючерсы ММВБ
    ...
    save_candles_to_file(ap_provider, board=board, symbols=symbols, tf='D1', skip_last_date=skip_last_date, four_price_doji=True)  # Дневные бары (с начала)
    # save_candles_to_file(ap_provider, board=board, symbols=symbols, tf='M60', skip_last_date=skip_last_date)  # Часовые бары (с 11.12.2017)
    # save_candles_to_file(ap_provider, board=board, symbols=symbols, tf='M15', skip_last_date=skip_last_date)  # 15-и минутные бары (с 11.12.2017)
    # save_candles_to_file(ap_provider, board=board, symbols=symbols, tf='M5', skip_last_date=skip_last_date)  # 5-и минутные бары (с 11.12.2017)
    # save_candles_to_file(ap_provider, board=board, symbols=symbols, tf='M1', skip_last_date=skip_last_date, four_price_doji=True)  # Минутные бары (с 11.12.2017)
